...
public class narayana2gen {
	...
	public static void main(String[] args) throws Exception {
		...
		for (n = 1; n < 10; n++) {//mn is length of path; m colors; n of each colors
    		...
    		long[] ret = generate(0, info, new long[m], nodes, 0);
    		int f = factorial(m);
	...
	static long[] generate(int cur, int[][] info, long[] ret, Node[] nodes, int level) {//can get rid of level in favor of nodes[cur].level but lazy
		...
		if (cur == n*m) {
			int k = simplifiedCount(countLeaves(0, nodes, new int[m+1]));
			ret[k-1]++;
	...
	static int[] countLeaves(int head, Node[] nodes, int[] ret) {
		if (nodes[head].children.size() == 0) ret[nodes[head].color]++;
		for (int nod : nodes[head].children) ret = countLeaves(nod, nodes, ret);
		return ret;
	}
	static int simplifiedCount(int[] vals) {
		int count = 0;
		for (int i = 0; i < vals.length; i++) if (vals[i] == n) count++;
		return count;
	}
